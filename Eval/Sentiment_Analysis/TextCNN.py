...
class TextCNN(object):
    ...
    def predict_on_batch(self, sess, feed, num, prob=True):
        print(sess.run(self.pred,feed_dict=feed))
        a=sess.run(self.pred,feed_dict=feed)
        np.savetxt('./data/'+str(num)+'.csv', a)
    ...
    def predict(self, sess, x, prob=False):
        ...
        for _feed, _ in self.get_batches(x, batch_size=self.batch_size, is_shuffle=False):
            _yhat = self.predict_on_batch(sess, _feed, num, prob)
            print(num)
            num+=1
            yhat += _yhat.tolist()
            # yhat.append(_yhat)
