...
build_word2id('./data/t_total.txt', './data/t_word_to_id.txt')
...
def build_word2vec(fname, word2id, save_to_path=None):
    ...
    vector_size = len(word_embedding)

    n_words = max(word2id.values()) + 1
    word_vecs = np.array(np.random.uniform(-1., 1., [n_words, vector_size]))
    for word in word2id.keys():
        try:
            word_vecs[word2id[word]] = word_embedding_dict[word]
        except KeyError:
            pass
    #
...
word2id = load_word2id('./data/t_word_to_id.txt')
w2v = build_word2vec('./data/cbow_after_word2vec.bin', word2id, save_to_path='./data/t_corpus_word2vec.txt')
