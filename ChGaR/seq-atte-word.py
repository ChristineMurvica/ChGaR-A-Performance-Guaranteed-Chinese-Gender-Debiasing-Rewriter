...
data_answer = pd.read_csv('data_answer_4.csv')
before=[line for line in data_answer['before'] if pd.isnull(line)==False]
after=[line for line in data_answer['after'] if pd.isnull(line)==False]

before_token_list = []
after_token_list = []
for s in before:
    before_token_list.append(s.split())
for s in after:
    after_token_list.append(s.split())

before_w_vocab = []
for i in before_token_list:
    before_w_vocab.extend(i)
before_c_vocab=set(''.join(before_w_vocab))

before_vocab=[]
before_vocab.extend(before_w_vocab)
before_vocab.extend(before_c_vocab)
before_vocab=list(set(before_vocab))

after_w_vocab = []
for i in after_token_list:
    after_w_vocab.extend(i)
after_c_vocab=set(''.join(after_w_vocab))

after_vocab=[]
after_vocab.extend(after_w_vocab)
after_vocab.extend(after_c_vocab)
after_vocab=list(set(after_vocab))

basic_dict = {'<pad>':0, '<unk>':1, '<bos>':2, '<eos>':3}
before2id = {word:i+len(basic_dict) for i, word in enumerate(before_vocab)}
before2id.update(basic_dict)
id2before = {v:k for k,v in before2id.items()}

after2id = {word:i+len(basic_dict) for i, word in enumerate(after_vocab)}
after2id.update(basic_dict)
id2after = {v:k for k,v in after2id.items()}

trans=data_answer[data_answer['trans_count']!=0]
notrans=data_answer[data_answer['trans_count']==0].sample(75000,replace=False,axis=0)

train_pd=pd.concat([trans[:75000],notrans],axis=0)
train_pd=train_pd.sample(frac=1)

train_before=[line for line in train_pd['before'] if pd.isnull(line)==False]
train_after=[line for line in train_pd['after'] if pd.isnull(line)==False]

before_train_list = []
after_train_list = []
for s in train_before:
    before_train_list.append(s.split()+['<eos>'])
for s in train_after:
    after_train_list.append(s.split()+['<eos>'])

before_num_data = [[before2id[b] for b in line ] for line in before_train_list]
after_num_data = [[after2id[a] for a in line] for line in after_train_list]

a=np.array(before_num_data)
np.save('before_num_data.npy',a)
b=np.array(after_num_data)
np.save('after_num_data.npy',b)
np.save('id2before.npy',id2before)
np.save('id2after.npy',id2after)
print("数据处理完毕")
...
#model.load_state_dict(torch.load('Areverse-model-9.pt'))
...
for epoch in range(N_EPOCHS):
    ...
    if valid_loss < best_valid_loss:
        ...
        torch.save(model.state_dict(), 'Aver-4modelPOS-256-256-512-' + str(epoch) + '.pt')
...
random.seed(seed)
for i in random.sample(range(len(before_num_data)), 10):
    before_tokens = list(filter(lambda x: x!=0, before_num_data[i]))
    after_tokens = list(filter(lambda x: x!=3 and x!=0, after_num_data[i]))
